...
```cpp
    ...
    bool Watchdog::attach(sol::state_view newLua, bool force /* = false */)
    {
        ...
        if (force) {
            ...
            sstd::cerr << "Cannot attach timeout watchdog to a new Lua state while it's armed\n";
            return false;
        }
        lua = newLua;
        return true;
...
Код в удобоваримой форме, пригодной к использованию, можно посмотреть [здесь](https://github.com/ipochto/luaSandboxing/tree/master/src).
