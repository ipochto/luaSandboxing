...
```
Причём в нашем распоряжении аж четыре типа прерываний на выбор. Допустимые значения `mask`:
```cpp
...
В отличие от аллокатора, Lua при вызове хука не предоставляет ему указатель на пользовательские данные:
```cpp
typedef void (*lua_Hook) (lua_State *L, lua_Debug *ar);
```
...
- Лямбду с захватом в качестве хука подсунуть не получится — она просто не проеобразуется к указателю на функцию.
...
guardCtxRegistry::set(lua, &guard);

// И ставим хук
lua_sethook(lua.lua_state, lua::timeoutGuard, LUA_MASKCOUNT, 10'000);
...
3. Безопасные замены функций, запускающих файлы со скриптами, с проверкой путей и запретом байткода.
4. Лимиты памяти через кастомный аллокатор с диагностикой переполнения.
...
Спасибо, что дочитали. Буду рад, если пригодится.

---

Код в удобоваримой форме можно посмотреть [здесь](https://github.com/ipochto/luaSandboxing/tree/master/src)
