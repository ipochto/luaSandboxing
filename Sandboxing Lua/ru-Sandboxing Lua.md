...
В замечательном цикле статей Game++ от @dalerank, а если быть конкретней, то в [Game++. while (!game(over))](https://habr.com/ru/articles/907146/) есть абзац про скрипты в игровых движках, который меня заставил надолго задуматься.
...
К сожалению ответ на этот вопрос выходил далеко за рамки статьи, поэтому пришлось разбираться самому.
...
<details>
<summary> Так, здесь будет вынужденное отступление от основной темы. А именно, по поводу применения в геймдеве типов активно использующих динамическую аллокациею памяти. [развернуть]</summary>
