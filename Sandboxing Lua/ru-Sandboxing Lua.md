...
2. Hotreload — по факту изменения. В общем, логика простая — регистрируем определённые скрипты как требующие горячей перезагрузки, периодически проверяем, не изменились ли эти файлы, и, если да, то перезапускаем их.
